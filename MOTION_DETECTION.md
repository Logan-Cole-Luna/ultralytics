...
|---|---|---|---|---|---|---|---|
| **WBF base+hybrid** [^ensemble] | **0.619** | **0.364** | 0.407 | **0.821** | **0.618** | **0.960** | 15.5 |
| base [^base] | 0.577 | 0.336 | 0.449 | 0.783 | 0.556 | 0.885 | **7.4** |
| hybrid [^hybrid] | 0.538 | 0.321 | 0.418 | 0.789 | 0.571 | 0.937 | 8.2 |
| xattn [^xattn] | 0.542 | 0.329 | 0.491 | 0.751 | 0.512 | 0.862 | 8.3 |

[^base]: **base** — plain YOLO26s, no motion stream, trained with mosaic augmentation. The single-stream baseline every other row is measured against.
[^xattn]: **xattn** — `MotionDetectionModel` with `MotionCrossAttention` at all three injection points (P3/P4/P5); the P3 tap uses 4× spatial-reduction (SR) pooling on the motion K/V for tractable attention cost, P4/P5 are dense. The original cross-attention design: current-frame features as queries, motion-encoder features as keys/values.
...
[^ensemble]: **Ensemble (WBF = Weighted Box Fusion)** — `base` and `hybrid` run independently on every tile; their raw detections (confidence > 0.01) are fused per tile: boxes from both models are greedily clustered by mutual IoU ≥ 0.55, each cluster's box coordinates are confidence-weighted, and the cluster's final score is combined via noisy-OR (`1 − ∏(1 − confᵢ)`) rather than averaged, so agreement between the two models pushes borderline detections higher instead of diluting a confident one. Ensembling costs the sum of both models' latency and inference passes. (This is a documented result; the standalone evaluation/fusion harness was removed in the cleanup to hybrid-only and can be re-added when needed.)

*All rows were measured on one shared prediction cache and box-matching protocol, so mAP/precision/recall are directly comparable across rows — training-run `results.csv` values (letterboxed/rect val) differ slightly and should not be mixed with this table.*
